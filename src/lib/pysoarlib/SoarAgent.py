...
class SoarAgent():
    ...
    def __init__(self, print_handler=None, config_filename=None, root='', **kwargs):
        ...
        root_dir = [string] 
            Where to look for the agent-source file if the config file is specified.
        ...
        self.root_dir = self.settings['root_dir'] if 'root_dir' in self.settings else ''
        # Force a trailing slash
        self.root_dir = self.root_dir + \
            '/' if self.root_dir != '' and self.root_dir[-1] != '/' else self.root_dir
        self.config_filename = self.root_dir + \
            config_filename if config_filename else None
...
# Internal Methods
        ...
        try:
            ...
            for args in config_args:
                if len(args) == 3 and args[1] == '=':
                    ...
                    if key == 'agent_source':
                        self.settings[key] = self.root_dir + args[2]
        except IOError:
            pass
    ...
    def _apply_settings(self):
        ...
        self.remote_connection = self._parse_bool_setting(
        ...
        self.write_to_stdout = self._parse_bool_setting(
        ...
        self.use_time_connector = self._parse_bool_setting(
            "use_time_connector", False)
    ...
    def _create_soar_agent(self):
        ...
        if self.enable_log:
            try:
                ...
                self.print_handler(
                    "ERROR: Cannot open log file " + self.log_filename)
    ...
    def _source_agent(self):
        ...
        if self.agent_source != None:
            ...
            self.print_handler(
                "agent_source not specified, no rules are being sourced")
